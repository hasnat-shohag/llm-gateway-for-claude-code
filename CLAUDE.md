...
| Write no gateway credential when a login exists **and** an enabled `passthrough` provider can use it | `claude-settings.js` | The *absence* of `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_API_KEY` is what keeps the subscription active, which is what makes a `passthrough` provider work. A login alone is not enough to withhold it: Claude Code with a custom base URL and no credential in `env` runs its login flow instead of using the gateway. |
| A provider save rewrites `~/.claude/settings.json` when routing is on | `ipc.js` (`providers:save`) | Enabling or disabling a `passthrough` provider flips whether the placeholder credential belongs there. |
